...
def test_switchStatsSubtab_loads_table():
    ...
    panel_path = Path(__file__).parent.parent / "custom_components" / "pill_assistant" / "www" / "pill-assistant-panel.html"
    ...
    assert 'loadMedicationHistory()' in function_body, \
    ...
    assert "subtab === 'table'" in function_body or 'subtab === "table"' in function_body, \
...
def test_setViewContext_reloads_table_when_active():
    ...
    assert "currentStatsSubtab === 'table'" in function_body or 'currentStatsSubtab === "table"' in function_body, \
...
def test_table_loads_after_renderStatistics():
    ...
    assert load_history_pos > render_pos, \
...
def test_currentStatsSubtab_variable_exists():
    ...
    assert re.search(r"let currentStatsSubtab\s*=\s*['\"]graphs['\"]", content), \
...
def test_table_auto_load_preserves_date_range():
    ...
    assert "getElementById('stats-start-date')" in function_body or 'getElementById("stats-start-date")' in function_body, \
        "loadMedicationHistory should read start date from date picker"
    assert "getElementById('stats-end-date')" in function_body or 'getElementById("stats-end-date")' in function_body, \
...
def test_table_respects_medication_context():
    ...
    assert 'currentViewContext' in function_body or 'currentMedicationId' in function_body, \
